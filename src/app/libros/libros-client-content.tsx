...
function LibroForm({ currentData, initialValues, onSubmit, onCancel, isSubmitting, editoriales }: LibroFormProps) {
  ...
  useEffect(() => {
    ...
    if (initialValues && Object.keys(initialValues).length > 0) {
      ...
      form.reset({
        ...
        anioPublicacion: currentData.anioPublicacion.toString(),
  ...
  return (
    <Card className="max-w-2xl mx-auto shadow-lg rounded-lg">
      <CardHeader><CardTitle className="text-2xl font-semibold text-primary">{currentData ? 'Editar Libro' : (initialValues?.titulo ? 'Crear Libro (Sugerencias IA)' : 'Crear Nuevo Libro')}</CardTitle></CardHeader>
      <Form {...form}>
...
function LibroList({ items, onEdit, onDelete }: LibroListProps) {
  return (
    <Card className="shadow-md rounded-lg">
      <CardHeader><CardTitle className="text-xl font-semibold text-primary">Lista de Libros</CardTitle></CardHeader>
      <CardContent>
        {items.length === 0 ? (
...
export default function LibrosClientContent() {
  ...
  if (loading && !showForm && filteredData.length === 0 && !searchTerm) return (
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-primary flex items-center gap-3"><BookOpen className="h-8 w-8" />Gestión de Libros</h1>
